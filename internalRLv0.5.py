...
version 0.4
Version modified for Journal paper with Y. Nagai part 1, for sections
choosing and advisor and comparing advisor and learner behavior. For 
Evaluation of System parameters see version 0.5.

version 0.5
For Evaluation of System parameters section of paper with Y. Nagai
...
class Experiment(object):
    ...
    def trainLearners(self, feedback, consistency=1, follow=1):
        ...
        #Loading teacher        
        [bestTeacher, v1, v2, v3] = self.selectBestAgent('visitedStatesRL.csv')
    ...
    def selectBestAgent(self, filename):
        filename = self.resultsFolder + filename
    ...
    def selectWorstAgent(self, filename):
        filename = self.resultsFolder + filename
        dataRL = np.genfromtxt(filename, delimiter=',')
        mean = np.mean(dataRL, axis=1)
        std = np.std(dataRL, axis=1)
...
if __name__ == "__main__":
    ...
    feedback = 0.25
    results = 'results/July6/Feedback0.25/'
    agents = 100
    episodes = 3000
    experiment = Experiment(results, agents, episodes)
    
    #experiment.trainTeachers()
    print experiment.selectBestAgent('visitedStatesRL.csv')
    print experiment.selectWorstAgent('visitedStatesRL.csv')

    #experiment.trainLearners(feedback)
    #print experiment.selectBestAgent('C1F1visitedStatesIRL.csv')
    #print experiment.selectWorstAgent('C1F1visitedStatesIRL.csv')

    
    for j in frange(0.8,0.95,0.05):
    #for j in frange(0.0,1.0,0.25):
        for i in frange(0.0,1.0,0.25):
            ...
            experiment.trainLearners(feedback, j,i)
